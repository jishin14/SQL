...
SELECT MIN(HIRE_DATE), MAX(HIRE_DATE), MIN(FIRST_NAME), MAX(FIRST_NAME)
FROM EMPLOYEES;
...
--부서아이디가 NULL이 아닌 데이터 중에서 입사일은 05년도인 사람들의 부서별 급여평균, 급여합을 구하고,
...
--GROUPING함수: GROUP BY로 만들어진 경우는 0반환, 롤업 또는 큐브로 만들어진 행인 경우는 1을 반환
...
SELECT JOB_ID, COUNT(*), AVG(SALARY) AS 평균
FROM EMPLOYEES
GROUP BY JOB_ID
ORDER BY 평균 DESC;
...
SELECT TO_CHAR(HIRE_DATE, 'YY'), COUNT(*) AS 사원수
FROM EMPLOYEES
...
SELECT DEPARTMENT_ID, AVG(SALARY) AS 평균, SUM(SALARY) AS 합계
FROM EMPLOYEES
